...
Shader "Silent/Volumetric Fake Lights (Projector Smoke)"
{
    Properties
    {
        ...
        [NoScaleOffset] _Tex ("Smoke", CUBE) = "white" {}
        [Space]
...
CBUFFER_END
        ...
        Pass
        {
            ...
            float3x3 LookAt(float3 direction, float3 up)
            {
                float3 zAxis = normalize(direction);
                float3 xAxis = normalize(cross(up, zAxis));
                float3 yAxis = cross(zAxis, xAxis);

                float3x3 rotationMatrix = float3x3(xAxis, yAxis, zAxis);
            ...
            fixed4 pixel_shader(v2f ps ) : SV_TARGET
            {
                ...
                float4 localViewDirection = mul(unity_WorldToObject, viewDirection);
                float centeredness = (dot(normalize(viewDirection), normalize(baseWorldPos - ps.world_vertex)) + 1) / 2;

                float4 localBaseDirection = mul(unity_WorldToObject, baseWorldPos - _WorldSpaceCameraPos);
                half3 midSwirlUV = MirrorVector(ps.vertex, localViewDirection.xyz);
                half3 mirroredLocalViewDirection = MirrorVector(localViewDirection, localBaseDirection); 
                half3 midSwirlUV2 = lerp(midSwirlUV, mirroredLocalViewDirection, (1 - centeredness));
                half4 midSwirl = 1;
                float height = texCUBE(_Tex, Unity_RotateAboutAxis_Radians_float(midSwirlUV, float3(sin(_Time.x),cos(_Time.x),sin(_Time.x + 2)), 1.5 * sin(_Time.x))).x;
                midSwirlUV2 = lerp(midSwirlUV, midSwirlUV2, -(1 - (height * height)) / 8);
                midSwirl.xyz = texCUBE(_Tex, Unity_RotateAboutAxis_Radians_float(midSwirlUV2, float3(sin(_Time.x),cos(_Time.x),sin(_Time.x + 2)), 1.5 * sin(_Time.x)));
                midSwirl.a = saturate((length(depthPosition - baseWorldPos) - (1 - midSwirl.x) / 10) * 25) * (1 - centeredness/2) * midSwirl.x * saturate((finalSphere - 10) / 10);
                midSwirl.xyz = 1;

                half3 bgSwirlUV = half3(0,0,1);
                float3x3 rotMatrix = LookAt(localBaseDirection, half3(0,1,0));
                bgSwirlUV = mul(rotMatrix, ps.vertex);
                bgSwirlUV = lerp(bgSwirlUV, mul(rotMatrix, localViewDirection), centeredness/2 * sin(_Time.y / 2));
                bgSwirlUV = Unity_RotateAboutAxis_Radians_float(bgSwirlUV, mul(rotMatrix, localBaseDirection), -centeredness * sin(_Time.y));
                float hardZClip = lerp(0, 1, sceneDepth > ps.projPos.z);
                half4 bgSwirl = texCUBE(_Tex, bgSwirlUV) * hardZClip;

                half3 c = DecodeHDR (lerp(0.5, (midSwirl.a + bgSwirl), hardZClip), _Tex_HDR);
                float vanishFac = getVanishingFactor(_VanishingStart, _VanishingEnd, 
                    ps.world_vertex.w);


                float finalAlpha = (finalSphere + finalLight) * vanishFac * (1 + midSwirl.a);
                float3 finalColor = finalLight + finalAlpha * ps.color * c;
